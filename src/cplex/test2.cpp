...
int main(){
	try{
		...
		for(int t = 0; t < T_MAX; t++){
			...
			c.add(-t +  s[6]      - T_MAX*(Y[8][t]-1)  >= 1);
		}
		c.add(sum2[8] - (s[6]-(s[4]+T)) == 0);

		sum2.add(IloExpr(env));
		for(int t = 0; t < T_MAX; t++){
			Y[9].add(IloBoolVar(env));
			sum2[9] +=  Y[9][t];
			c.add( t - (s[4] + T) - T_MAX*(Y[9][t]-1)  >= 0);
			c.add(-t +  s[7]      - T_MAX*(Y[9][t]-1)  >= 1);
		}
		c.add(sum2[9] - (s[7]-(s[4]+T)) == 0);
		...
		c.add(sum2[10] - (s[8]-(s[2]+T)) == 0);

		sum2.add(IloExpr(env));
		for(int t = 0; t < T_MAX; t++){
			Y[11].add(IloBoolVar(env));
			sum2[11] +=  Y[11][t];
			c.add( t - (s[2] + T) - T_MAX*(Y[11][t]-1)  >= 0);
			c.add(-t +  s[9]      - T_MAX*(Y[11][t]-1)  >= 1);
		}
		c.add(sum2[11] - (s[9]-(s[2]+T)) == 0);
		...
		c.add(sum2[12] - (s[10]-(s[6]+T)) == 0);

		sum2.add(IloExpr(env));
		for(int t = 0; t < T_MAX; t++){
			Y[14].add(IloBoolVar(env));
			sum2[14] +=  Y[14][t];
			c.add( t - (s[8] + T) - T_MAX*(Y[14][t]-1)  >= 0);
			c.add(-t +  s[11]      - T_MAX*(Y[14][t]-1)  >= 1);
		}
		c.add(sum2[14] - (s[11]-(s[8]+T)) == 0);

		sum2.add(IloExpr(env));
		for(int t = 0; t < T_MAX; t++){
			Y[17].add(IloBoolVar(env));
			sum2[17] +=  Y[17][t];
			c.add( t - (s[11] + T) - T_MAX*(Y[17][t]-1)  >= 0);
			c.add(-t +  s[7]      - T_MAX*(Y[17][t]-1)  >= 1);
		}
		c.add(sum2[17] - (s[7]-(s[11]+T)) == 0);

		sum2.add(IloExpr(env));
		for(int t = 0; t < T_MAX; t++){
			Y[18].add(IloBoolVar(env));
			sum2[18] +=  Y[18][t];
			c.add( t - (s[11] + T) - T_MAX*(Y[18][t]-1)  >= 0);
			c.add(-t +  s[9]      - T_MAX*(Y[18][t]-1)  >= 1);
		}
		c.add(sum2[18] - (s[9]-(s[11]+T)) == 0);
		...
		model.add(c);
		cplex.exportModel(model/test2.lp);
		cplex.solve();
