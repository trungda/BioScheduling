...
int main(){
	try{
		...
		for(int t = 0; t < T_MAX; t++){
			...
			c.add(-t +  s[4]      - T_MAX*(Y[4][t]-1)  >= 1);
		}
		c.add(sum2[4] - (s[4]-(s[3]+T)) == 0);
		...
		for(int t = 0; t < T_MAX; t++){
			...
			c.add(-t +  s[2]      - T_MAX*(Y[6][t]-1)  >= 1);
		}
		c.add(sum2[6] - (s[2]-(s[4]+T)) == 0);

		sum2.add(IloExpr(env));
		for(int t = 0; t < T_MAX; t++){
			Y[7].add(IloBoolVar(env));
			sum2[7] +=  Y[7][t];
			c.add( t - (s[5] + T) - T_MAX*(Y[7][t]-1)  >= 0);
			c.add(-t +  s[6]      - T_MAX*(Y[7][t]-1)  >= 1);
		}
		c.add(sum2[7] - (s[6]-(s[5]+T)) == 0);
		...
		for(int t = 0; t < T_MAX; t++){
			...
			c.add(-t +  s[6]      - T_MAX*(Y[9][t]-1)  >= 1);
		}
		c.add(sum2[9] - (s[6]-(s[2]+T)) == 0);
		...
		cplex.exportModel("../../model/ours_w.lp");
		cplex.solve();
		...
		start_time.emplace("O6", 2);
		start_time.emplace("O2", 3);
		start_time.emplace("O3", 3);
